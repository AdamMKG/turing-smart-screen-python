...
class LcdCommRevC(LcdComm):
    ...
    def auto_detect_com_port() -> Optional[str]:
        ...
        for com_port in com_ports:
            if (
                com_port.serial_number == "USB7INCH"
                or com_port.serial_number == "CT21INCH"
            ):
    ...
    def _get_awake_com_port(com_ports) -> Optional[str]:
        ...
        for com_port in com_ports:
            ...
            if com_port.vid == 0x0525 and com_port.pid == 0xA4A7:
                return com_port.device
            if com_port.vid == 0x1D6B and (
                com_port.pid == 0x0121 or com_port.pid == 0x0106
            ):
                return com_port.device
    ...
    def _hello(self):
        ...
        # Try to get the ID, but don't let it hang the whole program
        try:
            self._send_command(Command.HELLO, bypass_queue=True)
            raw_data = self.serial_read(23)
            response = "".join(
                filter(
                    lambda x: x in set(string.printable),
                    str(raw_data.decode(errors="ignore")),
                )
            )
            logger.debug(f"Display ID returned: {response}")
        except Exception as e:
            logger.warning(f"Could not read display ID: {e}")
            response = "unknown"

        # Reliable detection: Check dimensions regardless of orientation (Width vs Height)
        dims = {self.display_width, self.display_height}
        ...
        if dims == {480}:  # 480x480
            ...
            # Final fallback: If we can't tell, default to 5-inch so it at least tries to render
            logger.error(
                f"Unsupported resolution {self.display_width}x{self.display_height}. Defaulting to 5-inch."
            )
            self.sub_revision = SubRevision.REV_5INCH

        # Detect ROM version or default to 87
        try:
            parts = response.split(".")
            if len(parts) >= 3:
                self.rom_version = int(parts[2])
            else:
                self.rom_version = 87
        except:
            self.rom_version = 87

        logger.info(f"HW detected: {self.sub_revision}, ROM: {self.rom_version}")
    ...
    def DisplayPILImage(
        ...
        if (
            ...
            with self.update_queue_mutex:
                ...
                # Assign a default or handle the unknown state
                if self.sub_revision == SubRevision.REV_5INCH:
                    ...
                    # Fallback to prevent the UnboundLocalError
                    logger.warning(
                        "Sub-revision unknown at render time, using 5-inch default"
                    )
                    display_bmp_cmd = Command.DISPLAY_BITMAP_5INCH
                ...
                self._send_command(
                    display_bmp_cmd,
                    payload=bytearray(
                        int(self.display_width * self.display_width / 64).to_bytes(
                            2, "big"
                        )
                    ),
                )
    ...
    def _generate_update_image(
        ...
        for h, line in enumerate(chunked(img_data, image.width * pixel_size)):
            if self.sub_revision == SubRevision.REV_8INCH:
                # Switch landscape/portrait mode for 8"
                img_raw_data += int(((x0 + h) * self.display_width) + y0).to_bytes(
                    3, "big"
                )
            else:
