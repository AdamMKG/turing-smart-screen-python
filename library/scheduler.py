    ...
    def decorator(func):
        ...
        def async_func(*args, **kwargs):
            ...
            func_hl = threading.Thread(
                target=func, name=threadname, args=args, kwargs=kwargs
            )
            func_hl.start()
            return func_hl
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"]["CPU"]["PERCENTAGE"].get("INTERVAL", 0)
    ).total_seconds()
)
def CPUPercentage():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"]["CPU"]["FREQUENCY"].get("INTERVAL", 0)
    ).total_seconds()
)
def CPUFrequency():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"]["CPU"]["LOAD"].get("INTERVAL", 0)
    ).total_seconds()
)
def CPULoad():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"]["CPU"]["TEMPERATURE"].get("INTERVAL", 0)
    ).total_seconds()
)
def CPUTemperature():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"]["CPU"]["FAN_SPEED"].get("INTERVAL", 0)
    ).total_seconds()
)
def CPUFanSpeed():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("GPU", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def GpuStats():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("MEMORY", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def MemoryStats():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("DISK", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def DiskStats():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("NET", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def NetStats():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("DATE", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def DateStats():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("UPTIME", {}).get("INTERVAL", 0)
    ).total_seconds()
)
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("CUSTOM", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def CustomStats():
...
@schedule(
    timedelta(
        seconds=max(
            300.0, config.THEME_DATA["STATS"].get("WEATHER", {}).get("INTERVAL", 0)
        )
    ).total_seconds()
)
def WeatherStats():
...
@schedule(
    timedelta(
        seconds=config.THEME_DATA["STATS"].get("PING", {}).get("INTERVAL", 0)
    ).total_seconds()
)
def PingStats():
    ...
    if STOPPING:
        ...
        while not config.update_queue.empty():
            try:
                ...
                logger.error("QueueHandler error during shutdown: %s" % str(e))
    else:
        # Execute first action in the queue
        try:
            f, args = config.update_queue.get(timeout=1)
            if f:
                f(*args)
        except queue.Empty:
            # Timed out waiting for a queue item — loop back via the scheduler
            pass
        except Exception as e:
            # Log and survive: serial errors after sleep/wake must not kill this thread,
            # otherwise no further display updates will ever be processed
            logger.error("QueueHandler error processing queue item: %s" % str(e))
