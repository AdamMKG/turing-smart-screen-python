...
# Add a function to get the username to be called by the display class
def get_current_username_for_display():
    try:
...
class Display:
    ...
    def initialize_display(self):
        """Initializes the device and ensures it is 'on'."""
        logger.info("Initializing hardware interface...")
        # Initialize communication and detect screen properties
        self.lcd.InitializeComm()
        # Set the orientation from theme
        self.lcd.SetOrientation(orientation=_get_theme_orientation())
        # Turn on the display with proper brightness
        self.turn_on()

    def turn_off(self):
        """Simulate power off by setting brightness to 0."""
        try:
            logger.info("Fading screen to black (Brightness: 0)...")
            # Ensure self.lcd exists before calling
            if hasattr(self, "lcd"):
